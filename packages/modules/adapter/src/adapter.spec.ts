...
import { createAdapter, adapterConfigSchema, adapterModule } from './adapter.module';

/*
 * `@trustsystem/module-adapter` is a contract wrapper.
 *
...
describe('adapter configuration validation', () => {
  it('installs with no configuration at all', () => {
    expect(adapterConfigSchema.parse({})).toEqual({ enabled: true });
  });
  ...
  it('rejects an unknown configuration key rather than ignoring it', () => {
    ...
    expect(adapterConfigSchema.safeParse({ enable: false }).success).toBe(false);
  });
  ...
  it('rejects a non-boolean enabled rather than coercing it', () => {
    ...
    expect(adapterConfigSchema.safeParse({ enabled: 'false' }).success).toBe(false);
  });
});
...
describe('adapter tenant isolation', () => {
   ...
   * organization filter could escape through. Isolation for the adapter domain is enforced,
  ...
  it('mounts no route of its own', () => {
    expect(adapterModule.routes).toEqual([]);
  });

  it('initializes with no database client at all', async () => {
    const { context } = createTestModuleContext(adapterModule, { prisma: null });

    await expect(createAdapter(context).initialize()).resolves.toBeUndefined();
  });
  ...
  it('declares audit events, because it fronts something that changes state', () => {
    ...
    expect(adapterModule.auditEvents.length).toBeGreaterThan(0);
  });
});
...
describe('adapter lifecycle', () => {
  it('is ready and healthy after initialize', async () => {
    const { context } = createTestModuleContext(adapterModule, { prisma: null });
    const instance = createAdapter(context);
  ...
  it('stays unready and reports degraded when disabled by configuration', async () => {
    const { context } = createTestModuleContext(adapterModule, {
      prisma: null,
  ...
  it('tolerates shutdown without a preceding initialize', async () => {
    ...
    await expect(createAdapter(context).shutdown()).resolves.toBeUndefined();
  });
});
