...
import { createImport, importConfigSchema, importModule } from './import.module';

/*
 * `@trustsystem/module-import` is a contract wrapper.
 *
...
describe('import configuration validation', () => {
  it('installs with no configuration at all', () => {
    expect(importConfigSchema.parse({})).toEqual({ enabled: true });
  });
  ...
  it('rejects an unknown configuration key rather than ignoring it', () => {
    ...
    expect(importConfigSchema.safeParse({ enable: false }).success).toBe(false);
  });
  ...
  it('rejects a non-boolean enabled rather than coercing it', () => {
    ...
    expect(importConfigSchema.safeParse({ enabled: 'false' }).success).toBe(false);
  });
});

describe('import tenant isolation', () => {
  /*
   * This package reaches no database and mounts no route, so it owns no query a missing
   * organization filter could escape through. Isolation for the import domain is enforced,
  ...
  it('mounts no route of its own', () => {
    expect(importModule.routes).toEqual([]);
  });

  it('initializes with no database client at all', async () => {
    const { context } = createTestModuleContext(importModule, { prisma: null });

    await expect(createImport(context).initialize()).resolves.toBeUndefined();
  });
  ...
  it('declares audit events, because it fronts something that changes state', () => {
    ...
    expect(importModule.auditEvents.length).toBeGreaterThan(0);
  });
});
...
describe('import lifecycle', () => {
  it('is ready and healthy after initialize', async () => {
    const { context } = createTestModuleContext(importModule, { prisma: null });
    const instance = createImport(context);
  ...
  it('stays unready and reports degraded when disabled by configuration', async () => {
    const { context } = createTestModuleContext(importModule, {
      prisma: null,
  ...
  it('tolerates shutdown without a preceding initialize', async () => {
    ...
    await expect(createImport(context).shutdown()).resolves.toBeUndefined();
  });
});
