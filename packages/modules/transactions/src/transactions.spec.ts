...
describe('transactions configuration validation', () => {
  it('installs with no configuration at all', () => {
    expect(transactionsConfigSchema.parse({})).toEqual({ enabled: true });
  });
  ...
  it('rejects an unknown configuration key rather than ignoring it', () => {
    ...
    expect(transactionsConfigSchema.safeParse({ enable: false }).success).toBe(false);
  });
  ...
  it('rejects a non-boolean enabled rather than coercing it', () => {
    ...
    expect(transactionsConfigSchema.safeParse({ enabled: 'false' }).success).toBe(false);
  });
});
...
describe('transactions tenant isolation', () => {
   ...
   * organization filter could escape through. Isolation for the transactions domain is enforced,
  ...
  it('mounts no route of its own', () => {
    expect(transactionsModule.routes).toEqual([]);
  });

  it('initializes with no database client at all', async () => {
    const { context } = createTestModuleContext(transactionsModule, { prisma: null });

    await expect(createTransactions(context).initialize()).resolves.toBeUndefined();
  });
  ...
  it('declares audit events, because it fronts something that changes state', () => {
    ...
    expect(transactionsModule.auditEvents.length).toBeGreaterThan(0);
  });
});
...
describe('transactions lifecycle', () => {
  it('is ready and healthy after initialize', async () => {
    const { context } = createTestModuleContext(transactionsModule, { prisma: null });
    const instance = createTransactions(context);
  ...
  it('stays unready and reports degraded when disabled by configuration', async () => {
    const { context } = createTestModuleContext(transactionsModule, {
      prisma: null,
  ...
  it('tolerates shutdown without a preceding initialize', async () => {
    ...
    await expect(createTransactions(context).shutdown()).resolves.toBeUndefined();
  });
});
