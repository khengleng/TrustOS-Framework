...
import { createSync, syncConfigSchema, syncModule } from './sync.module';

/*
 * `@trustsystem/module-sync` is a contract wrapper.
 *
...
describe('sync configuration validation', () => {
  it('installs with no configuration at all', () => {
    expect(syncConfigSchema.parse({})).toEqual({ enabled: true });
  });
  ...
  it('rejects an unknown configuration key rather than ignoring it', () => {
    ...
    expect(syncConfigSchema.safeParse({ enable: false }).success).toBe(false);
  });
  ...
  it('rejects a non-boolean enabled rather than coercing it', () => {
    ...
    expect(syncConfigSchema.safeParse({ enabled: 'false' }).success).toBe(false);
  });
});
...
describe('sync tenant isolation', () => {
   ...
   * organization filter could escape through. Isolation for the sync domain is enforced,
  ...
  it('mounts no route of its own', () => {
    expect(syncModule.routes).toEqual([]);
  });

  it('initializes with no database client at all', async () => {
    const { context } = createTestModuleContext(syncModule, { prisma: null });

    await expect(createSync(context).initialize()).resolves.toBeUndefined();
  });
  ...
  it('declares audit events, because it fronts something that changes state', () => {
    ...
    expect(syncModule.auditEvents.length).toBeGreaterThan(0);
  });
});
...
describe('sync lifecycle', () => {
  it('is ready and healthy after initialize', async () => {
    const { context } = createTestModuleContext(syncModule, { prisma: null });
    const instance = createSync(context);
  ...
  it('stays unready and reports degraded when disabled by configuration', async () => {
    const { context } = createTestModuleContext(syncModule, {
      prisma: null,
  ...
  it('tolerates shutdown without a preceding initialize', async () => {
    ...
    await expect(createSync(context).shutdown()).resolves.toBeUndefined();
  });
});
