...
import { createWallet, walletConfigSchema, walletModule } from './wallet.module';

/*
 * `@trustsystem/module-wallet` is a contract wrapper.
 *
...
describe('wallet configuration validation', () => {
  it('installs with no configuration at all', () => {
    expect(walletConfigSchema.parse({})).toEqual({ enabled: true });
  });
  ...
  it('rejects an unknown configuration key rather than ignoring it', () => {
    ...
    expect(walletConfigSchema.safeParse({ enable: false }).success).toBe(false);
  });
  ...
  it('rejects a non-boolean enabled rather than coercing it', () => {
    ...
    expect(walletConfigSchema.safeParse({ enabled: 'false' }).success).toBe(false);
  });
});
...
describe('wallet tenant isolation', () => {
   ...
   * organization filter could escape through. Isolation for the wallet domain is enforced,
  ...
  it('mounts no route of its own', () => {
    expect(walletModule.routes).toEqual([]);
  });

  it('initializes with no database client at all', async () => {
    const { context } = createTestModuleContext(walletModule, { prisma: null });

    await expect(createWallet(context).initialize()).resolves.toBeUndefined();
  });
  ...
  it('declares audit events, because it fronts something that changes state', () => {
    ...
    expect(walletModule.auditEvents.length).toBeGreaterThan(0);
  });
});
...
describe('wallet lifecycle', () => {
  it('is ready and healthy after initialize', async () => {
    const { context } = createTestModuleContext(walletModule, { prisma: null });
    const instance = createWallet(context);
  ...
  it('stays unready and reports degraded when disabled by configuration', async () => {
    const { context } = createTestModuleContext(walletModule, {
      prisma: null,
  ...
  it('tolerates shutdown without a preceding initialize', async () => {
    ...
    await expect(createWallet(context).shutdown()).resolves.toBeUndefined();
  });
});
