...
describe('agent configuration validation', () => {
  it('installs with no configuration at all', () => {
    expect(agentConfigSchema.parse({})).toEqual({ enabled: true });
  });
  ...
  it('rejects an unknown configuration key rather than ignoring it', () => {
    ...
    expect(agentConfigSchema.safeParse({ enable: false }).success).toBe(false);
  });
  ...
  it('rejects a non-boolean enabled rather than coercing it', () => {
    ...
    expect(agentConfigSchema.safeParse({ enabled: 'false' }).success).toBe(false);
  });
});
...
describe('agent tenant isolation', () => {
   ...
   * organization filter could escape through. Isolation for the agent domain is enforced,
  ...
  it('mounts no route of its own', () => {
    expect(agentModule.routes).toEqual([]);
  });

  it('initializes with no database client at all', async () => {
    const { context } = createTestModuleContext(agentModule, { prisma: null });

    await expect(createAgent(context).initialize()).resolves.toBeUndefined();
  });
  ...
  it('declares audit events, because it fronts something that changes state', () => {
    ...
    expect(agentModule.auditEvents.length).toBeGreaterThan(0);
  });
});
...
describe('agent lifecycle', () => {
  it('is ready and healthy after initialize', async () => {
    const { context } = createTestModuleContext(agentModule, { prisma: null });
    const instance = createAgent(context);
  ...
  it('stays unready and reports degraded when disabled by configuration', async () => {
    const { context } = createTestModuleContext(agentModule, {
      prisma: null,
  ...
  it('tolerates shutdown without a preceding initialize', async () => {
    ...
    await expect(createAgent(context).shutdown()).resolves.toBeUndefined();
  });
});
