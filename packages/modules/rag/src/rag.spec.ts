...
import { createRag, ragConfigSchema, ragModule } from './rag.module';

/*
 * `@trustsystem/module-rag` is a contract wrapper.
 *
...
describe('rag configuration validation', () => {
  it('installs with no configuration at all', () => {
    expect(ragConfigSchema.parse({})).toEqual({ enabled: true });
  });
  ...
  it('rejects an unknown configuration key rather than ignoring it', () => {
    ...
    expect(ragConfigSchema.safeParse({ enable: false }).success).toBe(false);
  });
  ...
  it('rejects a non-boolean enabled rather than coercing it', () => {
    ...
    expect(ragConfigSchema.safeParse({ enabled: 'false' }).success).toBe(false);
  });
});
...
describe('rag tenant isolation', () => {
   ...
   * organization filter could escape through. Isolation for the rag domain is enforced,
  ...
  it('mounts no route of its own', () => {
    expect(ragModule.routes).toEqual([]);
  });

  it('initializes with no database client at all', async () => {
    const { context } = createTestModuleContext(ragModule, { prisma: null });

    await expect(createRag(context).initialize()).resolves.toBeUndefined();
  });
  ...
  it('declares audit events, because it fronts something that changes state', () => {
    ...
    expect(ragModule.auditEvents.length).toBeGreaterThan(0);
  });
});
...
describe('rag lifecycle', () => {
  it('is ready and healthy after initialize', async () => {
    const { context } = createTestModuleContext(ragModule, { prisma: null });
    const instance = createRag(context);
  ...
  it('stays unready and reports degraded when disabled by configuration', async () => {
    const { context } = createTestModuleContext(ragModule, {
      prisma: null,
  ...
  it('tolerates shutdown without a preceding initialize', async () => {
    ...
    await expect(createRag(context).shutdown()).resolves.toBeUndefined();
  });
});
