...
import { createScheduler, schedulerConfigSchema, schedulerModule } from './scheduler.module';

/*
 * `@trustsystem/module-scheduler` is a contract wrapper.
 *
...
describe('scheduler configuration validation', () => {
  it('installs with no configuration at all', () => {
    expect(schedulerConfigSchema.parse({})).toEqual({ enabled: true });
  });
  ...
  it('rejects an unknown configuration key rather than ignoring it', () => {
    ...
    expect(schedulerConfigSchema.safeParse({ enable: false }).success).toBe(false);
  });
  ...
  it('rejects a non-boolean enabled rather than coercing it', () => {
    ...
    expect(schedulerConfigSchema.safeParse({ enabled: 'false' }).success).toBe(false);
  });
});
...
describe('scheduler tenant isolation', () => {
   ...
   * organization filter could escape through. Isolation for the scheduler domain is enforced,
  ...
  it('mounts no route of its own', () => {
    expect(schedulerModule.routes).toEqual([]);
  });

  it('initializes with no database client at all', async () => {
    const { context } = createTestModuleContext(schedulerModule, { prisma: null });

    await expect(createScheduler(context).initialize()).resolves.toBeUndefined();
  });
  ...
  it('declares audit events, because it fronts something that changes state', () => {
    ...
    expect(schedulerModule.auditEvents.length).toBeGreaterThan(0);
  });
});
...
describe('scheduler lifecycle', () => {
  it('is ready and healthy after initialize', async () => {
    const { context } = createTestModuleContext(schedulerModule, { prisma: null });
    const instance = createScheduler(context);
  ...
  it('stays unready and reports degraded when disabled by configuration', async () => {
    const { context } = createTestModuleContext(schedulerModule, {
      prisma: null,
  ...
  it('tolerates shutdown without a preceding initialize', async () => {
    ...
    await expect(createScheduler(context).shutdown()).resolves.toBeUndefined();
  });
});
