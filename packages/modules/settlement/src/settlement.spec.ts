...
describe('settlement configuration validation', () => {
  it('installs with no configuration at all', () => {
    expect(settlementConfigSchema.parse({})).toEqual({ enabled: true });
  });
  ...
  it('rejects an unknown configuration key rather than ignoring it', () => {
    ...
    expect(settlementConfigSchema.safeParse({ enable: false }).success).toBe(false);
  });
  ...
  it('rejects a non-boolean enabled rather than coercing it', () => {
    ...
    expect(settlementConfigSchema.safeParse({ enabled: 'false' }).success).toBe(false);
  });
});

describe('settlement tenant isolation', () => {
  /*
   * This package reaches no database and mounts no route, so it owns no query a missing
   * organization filter could escape through. Isolation for the settlement domain is enforced,
  ...
  it('mounts no route of its own', () => {
    expect(settlementModule.routes).toEqual([]);
  });

  it('initializes with no database client at all', async () => {
    const { context } = createTestModuleContext(settlementModule, { prisma: null });

    await expect(createSettlement(context).initialize()).resolves.toBeUndefined();
  });
  ...
  it('declares audit events, because it fronts something that changes state', () => {
    ...
    expect(settlementModule.auditEvents.length).toBeGreaterThan(0);
  });
});
...
describe('settlement lifecycle', () => {
  it('is ready and healthy after initialize', async () => {
    const { context } = createTestModuleContext(settlementModule, { prisma: null });
    const instance = createSettlement(context);
  ...
  it('stays unready and reports degraded when disabled by configuration', async () => {
    const { context } = createTestModuleContext(settlementModule, {
      prisma: null,
  ...
  it('tolerates shutdown without a preceding initialize', async () => {
    ...
    await expect(createSettlement(context).shutdown()).resolves.toBeUndefined();
  });
});
