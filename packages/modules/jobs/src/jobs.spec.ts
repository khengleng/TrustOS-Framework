...
import { createJobs, jobsConfigSchema, jobsModule } from './jobs.module';

/*
 * `@trustsystem/module-jobs` is a contract wrapper.
 *
...
describe('jobs configuration validation', () => {
  it('installs with no configuration at all', () => {
    expect(jobsConfigSchema.parse({})).toEqual({ enabled: true });
  });
  ...
  it('rejects an unknown configuration key rather than ignoring it', () => {
    ...
    expect(jobsConfigSchema.safeParse({ enable: false }).success).toBe(false);
  });
  ...
  it('rejects a non-boolean enabled rather than coercing it', () => {
    ...
    expect(jobsConfigSchema.safeParse({ enabled: 'false' }).success).toBe(false);
  });
});
...
describe('jobs tenant isolation', () => {
   ...
   * organization filter could escape through. Isolation for the jobs domain is enforced,
  ...
  it('mounts no route of its own', () => {
    expect(jobsModule.routes).toEqual([]);
  });

  it('initializes with no database client at all', async () => {
    const { context } = createTestModuleContext(jobsModule, { prisma: null });

    await expect(createJobs(context).initialize()).resolves.toBeUndefined();
  });
  ...
  it('declares audit events, because it fronts something that changes state', () => {
    ...
    expect(jobsModule.auditEvents.length).toBeGreaterThan(0);
  });
});
...
describe('jobs lifecycle', () => {
  it('is ready and healthy after initialize', async () => {
    const { context } = createTestModuleContext(jobsModule, { prisma: null });
    const instance = createJobs(context);
  ...
  it('stays unready and reports degraded when disabled by configuration', async () => {
    const { context } = createTestModuleContext(jobsModule, {
      prisma: null,
  ...
  it('tolerates shutdown without a preceding initialize', async () => {
    ...
    await expect(createJobs(context).shutdown()).resolves.toBeUndefined();
  });
});
