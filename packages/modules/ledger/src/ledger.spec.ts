...
import { createLedger, ledgerConfigSchema, ledgerModule } from './ledger.module';

/*
 * `@trustsystem/module-ledger` is a contract wrapper.
 *
...
describe('ledger configuration validation', () => {
  it('installs with no configuration at all', () => {
    expect(ledgerConfigSchema.parse({})).toEqual({ enabled: true });
  });
  ...
  it('rejects an unknown configuration key rather than ignoring it', () => {
    ...
    expect(ledgerConfigSchema.safeParse({ enable: false }).success).toBe(false);
  });
  ...
  it('rejects a non-boolean enabled rather than coercing it', () => {
    ...
    expect(ledgerConfigSchema.safeParse({ enabled: 'false' }).success).toBe(false);
  });
});
...
describe('ledger tenant isolation', () => {
   ...
   * organization filter could escape through. Isolation for the ledger domain is enforced,
  ...
  it('mounts no route of its own', () => {
    expect(ledgerModule.routes).toEqual([]);
  });

  it('initializes with no database client at all', async () => {
    const { context } = createTestModuleContext(ledgerModule, { prisma: null });

    await expect(createLedger(context).initialize()).resolves.toBeUndefined();
  });
  ...
  it('declares audit events, because it fronts something that changes state', () => {
    ...
    expect(ledgerModule.auditEvents.length).toBeGreaterThan(0);
  });
});
...
describe('ledger lifecycle', () => {
  it('is ready and healthy after initialize', async () => {
    const { context } = createTestModuleContext(ledgerModule, { prisma: null });
    const instance = createLedger(context);
  ...
  it('stays unready and reports degraded when disabled by configuration', async () => {
    const { context } = createTestModuleContext(ledgerModule, {
      prisma: null,
  ...
  it('tolerates shutdown without a preceding initialize', async () => {
    ...
    await expect(createLedger(context).shutdown()).resolves.toBeUndefined();
  });
});
