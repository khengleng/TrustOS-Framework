...
describe('ai configuration validation', () => {
  it('installs with no configuration at all', () => {
    expect(aiConfigSchema.parse({})).toEqual({ enabled: true });
  });
  ...
  it('rejects an unknown configuration key rather than ignoring it', () => {
    ...
    expect(aiConfigSchema.safeParse({ enable: false }).success).toBe(false);
  });
  ...
  it('rejects a non-boolean enabled rather than coercing it', () => {
    ...
    expect(aiConfigSchema.safeParse({ enabled: 'false' }).success).toBe(false);
  });
});
...
describe('ai tenant isolation', () => {
   ...
   * organization filter could escape through. Isolation for the ai domain is enforced,
  ...
  it('mounts no route of its own', () => {
    expect(aiModule.routes).toEqual([]);
  });

  it('initializes with no database client at all', async () => {
    const { context } = createTestModuleContext(aiModule, { prisma: null });

    await expect(createAi(context).initialize()).resolves.toBeUndefined();
  });
  ...
  it('declares audit events, because it fronts something that changes state', () => {
    ...
    expect(aiModule.auditEvents.length).toBeGreaterThan(0);
  });
});
...
describe('ai lifecycle', () => {
  it('is ready and healthy after initialize', async () => {
    const { context } = createTestModuleContext(aiModule, { prisma: null });
    const instance = createAi(context);
  ...
  it('stays unready and reports degraded when disabled by configuration', async () => {
    const { context } = createTestModuleContext(aiModule, {
      prisma: null,
  ...
  it('tolerates shutdown without a preceding initialize', async () => {
    ...
    await expect(createAi(context).shutdown()).resolves.toBeUndefined();
  });
});
