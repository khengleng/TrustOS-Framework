...
import { createWebhook, webhookConfigSchema, webhookModule } from './webhook.module';

/*
 * `@trustsystem/module-webhook` is a contract wrapper.
 *
...
describe('webhook configuration validation', () => {
  it('installs with no configuration at all', () => {
    expect(webhookConfigSchema.parse({})).toEqual({ enabled: true });
  });
  ...
  it('rejects an unknown configuration key rather than ignoring it', () => {
    ...
    expect(webhookConfigSchema.safeParse({ enable: false }).success).toBe(false);
  });
  ...
  it('rejects a non-boolean enabled rather than coercing it', () => {
    ...
    expect(webhookConfigSchema.safeParse({ enabled: 'false' }).success).toBe(false);
  });
});
...
describe('webhook tenant isolation', () => {
   ...
   * organization filter could escape through. Isolation for the webhook domain is enforced,
  ...
  it('mounts no route of its own', () => {
    expect(webhookModule.routes).toEqual([]);
  });

  it('initializes with no database client at all', async () => {
    const { context } = createTestModuleContext(webhookModule, { prisma: null });

    await expect(createWebhook(context).initialize()).resolves.toBeUndefined();
  });
  ...
  it('declares audit events, because it fronts something that changes state', () => {
    ...
    expect(webhookModule.auditEvents.length).toBeGreaterThan(0);
  });
});
...
describe('webhook lifecycle', () => {
  it('is ready and healthy after initialize', async () => {
    const { context } = createTestModuleContext(webhookModule, { prisma: null });
    const instance = createWebhook(context);
  ...
  it('stays unready and reports degraded when disabled by configuration', async () => {
    const { context } = createTestModuleContext(webhookModule, {
      prisma: null,
  ...
  it('tolerates shutdown without a preceding initialize', async () => {
    ...
    await expect(createWebhook(context).shutdown()).resolves.toBeUndefined();
  });
});
