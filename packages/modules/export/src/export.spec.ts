...
import { createExport, exportConfigSchema, exportModule } from './export.module';

/*
 * `@trustsystem/module-export` is a contract wrapper.
 *
...
describe('export configuration validation', () => {
  it('installs with no configuration at all', () => {
    expect(exportConfigSchema.parse({})).toEqual({ enabled: true });
  });
  ...
  it('rejects an unknown configuration key rather than ignoring it', () => {
    ...
    expect(exportConfigSchema.safeParse({ enable: false }).success).toBe(false);
  });
  ...
  it('rejects a non-boolean enabled rather than coercing it', () => {
    ...
    expect(exportConfigSchema.safeParse({ enabled: 'false' }).success).toBe(false);
  });
});
...
describe('export tenant isolation', () => {
   ...
   * organization filter could escape through. Isolation for the export domain is enforced,
  ...
  it('mounts no route of its own', () => {
    expect(exportModule.routes).toEqual([]);
  });

  it('initializes with no database client at all', async () => {
    const { context } = createTestModuleContext(exportModule, { prisma: null });

    await expect(createExport(context).initialize()).resolves.toBeUndefined();
  });
  ...
  it('declares audit events, because it fronts something that changes state', () => {
    ...
    expect(exportModule.auditEvents.length).toBeGreaterThan(0);
  });
});
...
describe('export lifecycle', () => {
  it('is ready and healthy after initialize', async () => {
    const { context } = createTestModuleContext(exportModule, { prisma: null });
    const instance = createExport(context);
  ...
  it('stays unready and reports degraded when disabled by configuration', async () => {
    const { context } = createTestModuleContext(exportModule, {
      prisma: null,
  ...
  it('tolerates shutdown without a preceding initialize', async () => {
    ...
    await expect(createExport(context).shutdown()).resolves.toBeUndefined();
  });
});
