...
/**
 * Startup for the internal app gateway.
 *
...
async function bootstrap(): Promise<void> {
  ...
  const app = await NestFactory.create(
    InternalAppGatewayModule.forRoot({ config, policy, logger, environment }),
  ...
  if (config.observability.openApiEnabled) {
    const document = SwaggerModule.createDocument(
      app,
      new DocumentBuilder()
        .setTitle('TrustOS Internal App Gateway')
        .setDescription(
          'The single entrance every internal application calls. Two routes carry the traffic — ' +
            'data for reads and actions for everything else — and neither takes a path, a query ' +
            'or a resource id from the caller: both take a declared id from the application’s ' +
            'own definition, which is the thing security reviewed.',
        )
  ...
  logger.info(
    ...
    'trustos internal-app-gateway started',
  );

  logger.warn(
    {},
    'No resources are registered. Every read will be refused with "no approved resource" until ' +
      'a deployment registers its own — pass `resources` to InternalAppGatewayModule.forRoot.',
  );

  logger.warn(
    {},
    'This gateway forwards nothing. It plans, checks the declared operation and returns what ' +
      'would be called; wire a forwarder that calls the TrustOS API with the *actor’s* ' +
      'credential, never a service credential.',
  );
}
