...
function buildService(): { service: HospitalService; sink: InMemoryAuditSink } {
  const prisma = {
    hospitalDepartment: new FakeModelDelegate([
      {
        id: 'acme',
        organizationId: ACME,
        name: 'acme',
        code: 'hospital-department-acme',
        headPractitionerId: 'practitioner_acme',
        isActive: false,
        ...timestamps,
      },
      {
        ...
        name: 'rival',
        code: 'hospital-department-rival',
        headPractitionerId: 'practitioner_rival',
        isActive: false,
        ...timestamps,
      },
    ]),
    ward: new FakeModelDelegate([]),
    bed: new FakeModelDelegate([]),
    admission: new FakeModelDelegate([]),
  } as never;

  const sink = new InMemoryAuditSink();
  return { service: new HospitalService(prisma, new AuditService({ sink })), sink };
}
...
describe('hospital tenant isolation', () => {
  ...
  it('lists only the calling organization’s departments', async () => {
    expect((await asAcme(() => service.listHospitalDepartments())).map((row) => row.id)).toEqual([
      'acme',
    ]);
    expect((await asRival(() => service.listHospitalDepartments())).map((row) => row.id)).toEqual([
      'rival',
    ]);
  });
  ...
  it('reports another organization’s department as not_found', async () => {
    try {
      await asAcme(() => service.findHospitalDepartment('rival', ACME));
  ...
  it('stamps a new department with the calling organization', async () => {
    const created = await asAcme(() =>
      service.createHospitalDepartment(
        {
          name: 'new',
          code: 'hospital-department-new',
        } as never,
        ACME,
      ),
    );
  ...
  it('records an audit entry for the write', async () => {
    ...
    await asAcme(() =>
      service.createHospitalDepartment(
        {
          name: 'audited',
          code: 'hospital-department-audited',
        } as never,
        ACME,
      ),
    );
    ...
    expect(sink.records.map((record) => record.action)).toContain(
      'hospital.hospital-department.created',
    );
  });
});
