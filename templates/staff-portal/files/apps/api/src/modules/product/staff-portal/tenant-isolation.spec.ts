...
function buildService(): { service: StaffPortalService; sink: InMemoryAuditSink } {
  const prisma = {
    staffProfile: new FakeModelDelegate([
      {
        id: 'acme',
        organizationId: ACME,
        userId: 'acme',
    ...
    staffTask: new FakeModelDelegate([]),
    savedSearch: new FakeModelDelegate([]),
    staffNotification: new FakeModelDelegate([]),
  } as never;

  const sink = new InMemoryAuditSink();
  return { service: new StaffPortalService(prisma, new AuditService({ sink })), sink };
}
...
describe('staff-portal tenant isolation', () => {
  let service: StaffPortalService;
  ...
  it('lists only the calling organization’s staff', async () => {
    expect((await asAcme(() => service.listStaffProfiles())).map((row) => row.id)).toEqual([
      'acme',
    ]);
    expect((await asRival(() => service.listStaffProfiles())).map((row) => row.id)).toEqual([
      'rival',
    ]);
  });

  it('reports another organization’s staff member as not_found', async () => {
    try {
      await asAcme(() => service.findStaffProfile('rival', ACME));
  ...
  it('stamps a new staff member with the calling organization', async () => {
  ...
  it('records an audit entry for the write', async () => {
    ...
    expect(sink.records.map((record) => record.action)).toContain(
      'staffportal.staff-profile.created',
    );
  });
});
