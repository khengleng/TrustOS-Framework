...
function buildService(): { service: CustomerPortalService; sink: InMemoryAuditSink } {
  const prisma = {
    portalProfile: new FakeModelDelegate([
      {
        id: 'acme',
        organizationId: ACME,
        userId: 'acme',
    ...
    portalDocument: new FakeModelDelegate([]),
    portalNotification: new FakeModelDelegate([]),
    supportRequest: new FakeModelDelegate([]),
  } as never;

  const sink = new InMemoryAuditSink();
  return { service: new CustomerPortalService(prisma, new AuditService({ sink })), sink };
}
...
describe('customer-portal tenant isolation', () => {
  ...
  it('lists only the calling organization’s profiles', async () => {
    expect((await asAcme(() => service.listPortalProfiles())).map((row) => row.id)).toEqual([
      'acme',
    ]);
    expect((await asRival(() => service.listPortalProfiles())).map((row) => row.id)).toEqual([
      'rival',
    ]);
  });

  it('reports another organization’s profile as not_found', async () => {
    try {
      await asAcme(() => service.findPortalProfile('rival', ACME));
  ...
  it('stamps a new profile with the calling organization', async () => {
    const created = await asAcme(() =>
      service.createPortalProfile(
        {
          userId: 'new',
          displayName: 'new',
        } as never,
  ...
  it('records an audit entry for the write', async () => {
    ...
    expect(sink.records.map((record) => record.action)).toContain(
      'customerportal.portal-profile.created',
    );
  });
});
