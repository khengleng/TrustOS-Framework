...
function buildService(): { service: TelegramMiniappService; sink: InMemoryAuditSink } {
  const prisma = {
    miniAppUser: new FakeModelDelegate([
      {
        id: 'acme',
        organizationId: ACME,
        platform: 'TELEGRAM',
    ...
    miniAppSession: new FakeModelDelegate([]),
    deepLink: new FakeModelDelegate([]),
    menuEntry: new FakeModelDelegate([]),
    miniAppNotificationSetting: new FakeModelDelegate([]),
  } as never;

  const sink = new InMemoryAuditSink();
  return { service: new TelegramMiniappService(prisma, new AuditService({ sink })), sink };
}
...
describe('telegram-miniapp tenant isolation', () => {
  ...
  it('lists only the calling organization’s users', async () => {
    expect((await asAcme(() => service.listMiniAppUsers())).map((row) => row.id)).toEqual(['acme']);
    expect((await asRival(() => service.listMiniAppUsers())).map((row) => row.id)).toEqual([
      'rival',
    ]);
  });

  it('reports another organization’s user as not_found', async () => {
    try {
      await asAcme(() => service.findMiniAppUser('rival', ACME));
  ...
  it('stamps a new user with the calling organization', async () => {
    const created = await asAcme(() =>
      service.createMiniAppUser(
        {
          platform: 'TELEGRAM',
          platformUserId: 'new',
          displayName: 'new',
  ...
  it('records an audit entry for the write', async () => {
    ...
    expect(sink.records.map((record) => record.action)).toContain(
      'telegramminiapp.mini-app-user.created',
    );
  });
});
