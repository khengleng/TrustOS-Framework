...
function buildService(): { service: EcommerceService; sink: InMemoryAuditSink } {
  const prisma = {
    catalog: new FakeModelDelegate([
      {
        id: 'acme',
        organizationId: ACME,
        storeId: 'store_acme',
    ...
    product: new FakeModelDelegate([]),
    productVariant: new FakeModelDelegate([]),
    order: new FakeModelDelegate([]),
    orderLine: new FakeModelDelegate([]),
  } as never;

  const sink = new InMemoryAuditSink();
  return { service: new EcommerceService(prisma, new AuditService({ sink })), sink };
}
...
describe('ecommerce tenant isolation', () => {
  ...
  it('lists only the calling organization’s catalogs', async () => {
    expect((await asAcme(() => service.listCatalogs())).map((row) => row.id)).toEqual(['acme']);
    expect((await asRival(() => service.listCatalogs())).map((row) => row.id)).toEqual(['rival']);
  });

  it('reports another organization’s catalog as not_found', async () => {
    try {
      await asAcme(() => service.findCatalog('rival', ACME));
  ...
  it('stamps a new catalog with the calling organization', async () => {
  ...
  it('records an audit entry for the write', async () => {
    ...
    await asAcme(() =>
      service.createCatalog(
        {
          storeId: 'store_audited',
          name: 'audited',
          code: 'catalog-audited',
        } as never,
        ACME,
      ),
    );

    expect(sink.records.map((record) => record.action)).toContain('ecommerce.catalog.created');
  });
});
