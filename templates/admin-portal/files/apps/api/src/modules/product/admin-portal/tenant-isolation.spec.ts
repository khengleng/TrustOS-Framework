...
function buildService(): { service: AdminPortalService; sink: InMemoryAuditSink } {
  const prisma = {
    systemSetting: new FakeModelDelegate([
      {
        id: 'acme',
        organizationId: ACME,
        key: 'system-setting-acme',
        value: 'acme',
        description: 'acme',
        category: 'acme',
        isSecret: false,
    ...
    operatorNote: new FakeModelDelegate([]),
  } as never;

  const sink = new InMemoryAuditSink();
  return { service: new AdminPortalService(prisma, new AuditService({ sink })), sink };
}
...
describe('admin-portal tenant isolation', () => {
  ...
  it('lists only the calling organization’s configuration', async () => {
    expect((await asAcme(() => service.listSystemSettings())).map((row) => row.id)).toEqual([
      'acme',
    ]);
    expect((await asRival(() => service.listSystemSettings())).map((row) => row.id)).toEqual([
      'rival',
    ]);
  });

  it('reports another organization’s setting as not_found', async () => {
    try {
      await asAcme(() => service.findSystemSetting('rival', ACME));
  ...
  it('stamps a new setting with the calling organization', async () => {
    const created = await asAcme(() =>
      service.createSystemSetting(
        {
          key: 'system-setting-new',
          value: 'new',
          description: 'new',
          category: 'new',
        } as never,
  ...
  it('records an audit entry for the write', async () => {
    ...
    expect(sink.records.map((record) => record.action)).toContain(
      'adminportal.system-setting.created',
    );
  });
});
