...
function buildService(): { service: WhatsappMiniappService; sink: InMemoryAuditSink } {
  const prisma = {
    whatsAppProfile: new FakeModelDelegate([
      {
        id: 'acme',
        organizationId: ACME,
  ...
  return { service: new WhatsappMiniappService(prisma, new AuditService({ sink })), sink };
}
...
describe('whatsapp-miniapp tenant isolation', () => {
  let service: WhatsappMiniappService;
  ...
  it('lists only the calling organization’s whatsapp profiles', async () => {
    expect((await asAcme(() => service.listWhatsAppProfiles())).map((row) => row.id)).toEqual([
      'acme',
    ]);
    expect((await asRival(() => service.listWhatsAppProfiles())).map((row) => row.id)).toEqual([
      'rival',
    ]);
  });

  it('reports another organization’s whatsapp profile as not_found', async () => {
    try {
      await asAcme(() => service.findWhatsAppProfile('rival', ACME));
  ...
  it('stamps a new whatsapp profile with the calling organization', async () => {
    const created = await asAcme(() =>
      service.createWhatsAppProfile(
        {
  ...
  it('records an audit entry for the write', async () => {
    ...
    expect(sink.records.map((record) => record.action)).toContain(
      'whatsappminiapp.whats-app-profile.created',
    );
  });
});
