...
| ------------------------ | ------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------- |
| **Authentication**       | `@trustsystem/identity` — OIDC provider, token validation, `@HumanActorsOnly`              | None. The controller declares the decorator                              | **No**                                                                                         |
| **Tenant context**       | `@trustsystem/tenancy` — `@OrganizationId`, `scopedDelegate`                               | `assertTenanted`, which refuses an actor with no organization            | **No** — a guard against a caller that skips `toWorkflowActor`, not a second scoping mechanism |
| **RBAC**                 | `@trustsystem/rbac` — `@RequirePermissions`                                                | None                                                                     | **No**                                                                                         |
| **Authorization**        | `@trustsystem/authorization` — `@Authorize`, and the engine's own `authorizer.assert`      | None                                                                     | **No**                                                                                         |
| **Policy**               | `@trustsystem/policy-engine` + `@trustsystem/workflow-policy`, invoked inside `transition` | Projects `policyDecisionId` onto the decision view                       | **No**                                                                                         |
| **Workflow**             | `@trustsystem/workflow-runtime` — `WorkflowEngine.transition`, `available`, `find`, `list` | None. The only mutation available is `transition`                        | **No**                                                                                         |
| **Maker-checker**        | `@trustsystem/workflow-approvals` — eligibility and self-approval, enforced in the engine  | None                                                                     | **No**                                                                                         |
| **Tasks & eligibility**  | `@trustsystem/workflow-tasks` — `listAvailable`, `listMine`, `find`, `reassign`            | None. Eligibility is resolved server-side from roles and groups          | **No**                                                                                         |
| **Audit**                | `@trustsystem/audit` — `AuditService.query`, written by `HistoryRecorder`                  | Projects records into a timeline. Read-only                              | **No**                                                                                         |
...
| **Idempotency**          | `@trustsystem/workflow-runtime` `runIdempotent`                                            | Passes the client's key through                                          | **No**                                                                                         |
...
| **Approval view safety** | `@trustsystem/governance-workflow-bridge` — view staleness, capabilities                   | None                                                                     | **No**                                                                                         |
